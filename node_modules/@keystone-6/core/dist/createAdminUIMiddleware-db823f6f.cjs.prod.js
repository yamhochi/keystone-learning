...
var session_dist_keystone6CoreSession = require('../session/dist/keystone-6-core-session.cjs.prod.js');
var createApolloServer = require('./createApolloServer-91d74e5f.cjs.prod.js');
var url = require('url');
...
const createExpressServer = async (config, graphQLSchema, createContext) => {
  ...
  if ((_config$server3 = config.server) !== null && _config$server3 !== void 0 && _config$server3.extendExpressApp) {
    const createRequestContext = async (req, res) => createContext({
      sessionContext: config.session ? await session_dist_keystone6CoreSession.createSessionContext(config.session, req, res, createContext) : undefined,
      req
    });
  ...
  return async (req, res) => {
    ...
    try {
      ...
      const context = createContext({
        sessionContext: session ? await session_dist_keystone6CoreSession.createSessionContext(session, req, res, createContext) : undefined,
        req
      });
