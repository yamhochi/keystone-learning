...
async function setupTestEnv(_ref) {
  ...
  await migrations.pushPrismaSchemaToDatabase(config.db.url, config.db.shadowDatabaseUrl, artifacts.prisma, path__default["default"].join(artifactPath, 'schema.prisma'), true // shouldDropDatabase
  );
  const {
    connect,
    disconnect,
